...
void RegisterPerk()
{
    RTD2_ObtainPerk("bouncy") // create perk using unique token "mytoken"
        .SetName("Bouncy") // set perk's name
        .SetGood(false) // make the perk good
		.SetTime(0) //perk duration
        .SetSound("vo/scout_sf12_badmagic04.mp3") // set activation sound
        .SetClasses("0") // make the perk applicable only to Soldier, Pyro and Heavy
        .SetWeaponClasses("") // make the perk applicable only to clients holding a shotgun
        .SetTags("bad, bouncy, bounce, jump, mvm_bot") // set perk's search tags
